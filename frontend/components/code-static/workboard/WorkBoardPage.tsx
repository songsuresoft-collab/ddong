...
import type { WorkTask, Session, RulesetReport, Reference, TaskStatus, CompareResult, RuleSummary, RuleDelta } from '@/types/code-static';
import { getSessions, getRulesetReportsBySession, getReferences, saveTasks, getAllTasks, updateTaskStatus, clearTasksBySession } from '@/utils/code-static/db';
import { normalizeRuleName } from '@/utils/code-static/normalizer';
import { getPriorityLevel } from '@/utils/code-static/scoring';
import { generateWorkTasks, TASK_TYPE_LABELS } from '@/utils/code-static/taskGenerator';
import { findSimilarRules, type SimilarRule } from '@/utils/code-static/ruleSimilarity';
...
const STATUS_CONFIG: Record<TaskStatus, { label: string; color: string; bg: string }> = {
    TODO: { label: 'To Do', color: '#4318FF', bg: '#F4F7FE' },
    DOING: { label: 'Doing', color: '#01B574', bg: '#E6F9F0' },
    DONE: { label: 'Done', color: '#01B574', bg: '#E6F9F0' }, // Reusing Green for stability
    HOLD: { label: 'Hold', color: '#A3AED0', bg: '#F4F7FE' },
};

/* Redefining status config more dynamically */
const STATUS_STYLES: Record<TaskStatus, { color: string; bg: string; label: string }> = {
    TODO: { color: '#4318FF', bg: 'rgba(67, 24, 255, 0.1)', label: 'To Do' },
    DOING: { color: '#FFB547', bg: 'rgba(255, 181, 71, 0.1)', label: 'Doing' },
    DONE: { color: '#05CD99', bg: 'rgba(5, 205, 153, 0.1)', label: 'Done' },
    HOLD: { color: '#707EAE', bg: 'rgba(112, 126, 174, 0.1)', label: 'Hold' },
};
                ...
                <div className="workboard-status-bar">
                    {(['ALL', 'TODO', 'DOING', 'DONE', 'HOLD'] as const).map(s => {
                        ...
                        return (
                            ...
                            >
                                {s === 'ALL' ? `Total (${tasks.filter(t => t.sessionId === selectedSession).length})` : `${style!.label} (${statusCounts[s]})`}
                            </button>
                        );
                    })}
                </div>
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="workboard-card-footer">
                                        <div className="workboard-status-badge-container">
                                            <select
                                            ...
                                            >
                                                {(['TODO', 'DOING', 'DONE', 'HOLD'] as const).map(s => (
                                                    <option key={s} value={s} style={{ color: '#1B2559', background: '#fff' }}>
